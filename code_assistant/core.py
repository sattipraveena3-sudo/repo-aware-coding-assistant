...
def chunk_file(path: Path, root: Path) -> list[Chunk]:
    ...
    for node, qualified_name in _walk_definitions(tree):
        ...
        chunks.append(
            Chunk(
                id=f"{rel}:{node.lineno}:{qualified_name}",
                path=rel,
                symbol=node.name,
...
def retrieve(index: dict, question: str, k: int = 8) -> list[dict]:
    ...
    df = Counter(
        t
        for d in docs
        for t in set(tokens(f"{d['qualified_name']} {d.get('docstring', '')} {d['code']}"))
    )
    ...
    related = {
        rid
        for cid in list(ids)
        for rid in index.get("edges", {}).get(cid, []) + index.get("reverse_edges", {}).get(cid, [])
    }
...
def build_context(hits: list[dict], max_chars: int = 14000) -> str:
    ...
    for hit in hits:
        block = (
            f"FILE: {hit['path']}\n"
            f"SYMBOL: {hit['qualified_name']}\n"
            f"LINES: {hit['start']}-{hit['end']}\n"
            f"```python\n{hit['code']}\n```"
        )
...
def deterministic_answer(index: dict, question: str, k: int = 8) -> dict:
    ...
    if not hits:
        return {
            "answer": "No matching Python symbols were found in this repository index.",
            "references": [],
            "context": [],
            "mode": "retrieval",
        }
    ...
    for hit in hits[:6]:
        outgoing = [
            by_id[x]["qualified_name"]
            for x in index.get("edges", {}).get(hit["id"], [])
            if x in by_id
        ][:4]
        incoming = [
            by_id[x]["qualified_name"]
            for x in index.get("reverse_edges", {}).get(hit["id"], [])
            if x in by_id
        ][:4]
        relation = []
        if outgoing:
        ...
        lines.append(
            f"- {hit['path']}:{hit['start']}-{hit['end']} ({hit['qualified_name']}){suffix}"
        )
    return {
        "answer": "\n".join(lines),
